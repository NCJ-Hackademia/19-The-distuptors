...
CONFIDENCE_THRESHOLD = 3  # consecutive frames to confirm gesture
...
def get_finger_state(landmarks, tip, pip):
    return landmarks[tip].y < landmarks[pip].y

def get_thumb_state(landmarks, hand_side):
    if hand_side == "Right":
...
def main():
    ...
    with mp_hands.Hands(
        ...
        print("📷 Sign Language → Google Meet Translator")
        print("🤚 Show your hand to the camera")
        print("📋 Available gestures:", list(GESTURE_TO_TEXT.keys()))
        print("🔄 Press 'M' to open Meet and enable Meet mode")
        print("ESC to quit")
        ...
        while True:
            ...
            if keyboard.is_pressed('m'):
                if not meet_opened:
                    ...
                    webbrowser.open("https://meet.google.com/cuj-gjjb-pto")
                    time.sleep(10)
                    meet_opened = True
                meet_mode = True
                print("🔄 Meet mode ENABLED")
                time.sleep(1)
            ...
            if res.multi_hand_landmarks:
                ...
                g = recognize_gesture(handLms.landmark, handedness)
            ...
            if phrase_now and (phrase_now != last_phrase or (t - last_emit_ts) > EMIT_COOLDOWN_SEC):
                print(f"🗣 Detected: {phrase_now} ({gesture_name}, Confidence: {confidence:.1f}%)")
            ...
            cv2.putText(frame, "Sign Language → Google Meet", (30, 40),
            ...
            cv2.imshow("Sign Language → Google Meet", frame)
